<?php
    ...
    if(isset($_GET['addFIR'])){
        ...
        $model_name = preg_replace('!\s+!', ' ',$_POST['model_name']);
        ...
        // $fir_sql="SELECT  MAX(fir_id) FROM `fir`";   //6
        // $fir_result=$conn->query($fir_sql);
        // $fir_row=$fir_result->fetch_assoc();
        // $fir_id= $fir_row['MAX(fir_id)']+1;  //7
        ...
        if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
            ...
            if($conn->query($sql1)){
                ...
                $vehical_sql = "INSERT INTO vehical_fir(fir_id, plate_no, brand, model_name, model_year, color, type, location_of_theft, city_id, station_id, date_of_theft, time_of_theft, vehical_rc) VALUES ($fir_id,'$plate_no','$brand','$model_name','$model_year','$color','$type','$location_of_theft',$city,$police_station,'$date_of_theft','$time_of_theft','$imagename')";

                if($conn->query($vehical_sql)){
                    ...
                    echo "Error: " . $vehical_sql . "<br>" . mysqli_error($conn);
                }
